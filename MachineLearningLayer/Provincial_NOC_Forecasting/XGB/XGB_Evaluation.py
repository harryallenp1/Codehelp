...
class XGB_Evaluator(Evaluater):
    ...
    def Calculate_Regression_Metrics(self):
        try:
            ...
            print(f"XGB_Evaluator.Calculate_Regression_Metrics() => {e}")
            pass
    ...
    def Save_Metrics(self):
        try:
            if self.evaluation_data is not None:
                ...
                df.to_csv(f'{BASE_DIR}/XGB/Data/Evaluation_Metrics.csv', index=False)
                print("---Evaluation Metrics CSV File Saved---\n")

                profile = ProfileReport(df, title="XGB Evaluation Metrics Report", explorative=True)
                profile.to_file(f"{BASE_DIR}/XGB/Data/Reports/Evaluation_Metrics_Report.html")
...
if __name__ == '__main__':
    ...
    df_forecast = pl.scan_parquet(f'{BASE_DIR}/XGB/Data/All_Forecasts.parquet')

    forecast_data = df_forecast.filter(pl.col('set').is_in(['Validation','Test'])).collect().to_pandas()

    EvaluatorObj = XGB_Evaluator(forecast_data=forecast_data)
