...
class LGBM_PreProcessesor(PreProcessesor):
    def __init__(self):
        super().__init__()
        self.Data = None
    ...
    def EvaluateSetSizes(self, trainingYear, validationYear, testYear):
        ...
        ValSet_Count = ValSet.group_by(['provinceid','noc_groupingid']).agg(
            pl.col('datestamp').n_unique().alias('Validation Count')
        ).to_pandas()
    ...
    def SaveSample(self):
        self.Data.to_csv(f'{BASE_DIR}/LGBM/Data/PreProcessed_Data.csv',index=False)
